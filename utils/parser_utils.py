...
def get_args():
    ...
    if args.gpu_to_use == -1:
        args.use_cuda = False
    gpu_id = args.gpu_to_use

    os.environ["CUDA_VISIBLE_DEVICES"] = gpu_id  # sets the main GPU to be the one at index gpu id (on multi gpu machines you can choose which one you want to use by using the relevant GPU ID)
...
#         args_dict = extract_args_from_json(json_file_path=args_file, args_dict=args_dict)
#
#     if 'gpu_to_use' not in args_dict:
#         args_dict['gpu_to_use'] = str(-1)
#
#     for key in list(args_dict.keys()):
#
#         if str(args_dict[key]).lower() == "true":
...
#
#     os.environ[
#         "CUDA_VISIBLE_DEVICES"] = gpu_id  # sets the main GPU to be the one at index gpu id (on multi gpu machines you can choose which one you want to use by using the relevant GPU ID)
#     print(os.environ["CUDA_VISIBLE_DEVICES"])
#
#     import torch
#     from torch.cuda import _get_device_index
